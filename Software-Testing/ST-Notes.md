...
- 路径测试：关注程序结构但不关心数据
    ...
    - 循环测试的方法（假设循环执行 `N` 次）
        - 单循环测试：循环 `0, 1, 2, 某一中间值, N-1, N, N+1` 次（类似于健壮边界值测试）
...
- 程序片测试
    ...
    - 程序切片的前向，后向，静态，动态 [Book Ch9 P116] [Link: TODO]
    - 每次测试针对某一个变量集合的切片是否正确，如果所有变量的定义和使用都是正确的就可以认为程序整体是正确的
    - 能够比较便捷地定位错误（找出哪一个变量出错了）

- 黑盒测试与白盒测试的总结
    - 何时结束测试？ [PPT Ch7 P57]
    - 测试方法的取舍：trade-off
    - 对测试的评估：缺漏与冗余
        - 量化指标：假设执行方法M生成的 `m` 个测试用例时经过了测试指标S所指出的 `s` 个结构性测试单元中的 `n` 个，则：
